...
import { getOHLCV } from '../../../lib/monex.js'; // demo candles fallback
...
export async function POST(req) {
  ...
  // Support kedua format: brokerConfig baru { brokerId, credentials } atau lama { apiKey, ... }
  ...
  try {
    switch (action) {
      ...
      case 'cycle': {
        ...
        // FIX v3: fetch 200 candles agar EMA50/100/200 dan ADX akurat
        // Coba broker API dulu, fallback ke demo candles
        let candles = null;
        if (brokerConfig.brokerId !== 'demo') {
          try { candles = await brokerGetCandles(instrument, config?.tf || '5m', 200, brokerConfig); } catch {}
        }
        if (!candles || !candles.length) candles = await getOHLCV(instrument, granularity, 200, {});
