...
let botState = {
  ...
  // signalMode: 'scanner' | 'level' | 'combined'
  signalMode: 'combined',
...
export function startBot(cfg = {}) {
  ...
  botState.signalMode = cfg.signalMode || 'combined';
  botState.cooldownUntil  = 0;
  botState.lastActionTime = 0;
  botState.sessionSkipLogged = false;
  const modeLabel = botState.signalMode === 'scanner' ? '🔍 Scanner Only' : botState.signalMode === 'level' ? `📊 Level ${botState.level} Only` : `🤝 Combined L${botState.level}+Scanner`;
  addLog(`🚀 ForexBot started — ${botState.mode.toUpperCase()} | ${botState.instrument} | ${modeLabel}`, 'system');
}
...
export async function runCycle(candles, currentState = {}) {
  ...
  const signalMode = botState.signalMode || 'combined';
  ...
  if (signalMode === 'scanner') {
    ...
    if (scanSignal && scanSignal.action && scanSignal.action !== 'HOLD' && scanSignal.score >= 70) {
      ...
      addLog(`🔍 [Scanner Only] ${scanSignal.action} — Score ${scanSignal.score}`, 'system');
      ...
      addLog(`⏳ [Scanner Only] Score ${scanSignal?.score} < 70, tunggu sinyal lebih kuat`, 'info');
    } else {
    ...
    if (signal.action !== 'HOLD') {
      addLog(`📊 [Level ${botState.level} Only] ${signal.action} — Score ${signal.score}`, 'system');
    }
    ...
    if (scanSignal && scanSignal.action && scanSignal.action !== 'HOLD') {
      ...
      if (levelAction === scanSignal.action) {
        ...
        if (scanScore >= 85) {
          ...
          addLog(`⚡ Scanner ${scanSignal.action} (${scanScore}≥85) override konflik vs Level ${levelAction}`, 'warning');
        } else {
          signal = {
            ...signal,
            action:      'HOLD',
          ...
          addLog(`🚫 KONFLIK: Scanner ${scanSignal.action} (${scanScore}) vs Level ${levelAction} → HOLD`, 'warning');
        }
      }
    }
  }
      ...
      if (sizing.lots <= 0) {
        ...
        if (activeMode === 'max_profit') {
          ...
          if (atrRR >= 1.5) {
            ...
            addLog(`📊 [Max Profit] ATR adaptive SL/TP — R:R ${atrRR.toFixed(2)}x`, 'system');
          }
        }
