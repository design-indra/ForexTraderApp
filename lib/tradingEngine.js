...
export async function runCycle(candles, currentState = {}) {
  ...
  if (!session.isGood && openPositions.length === 0) {
    ...
    // Simpan lastSignal tetap agar tab signal tidak kosong
    if (!botState.lastSignal) {
      botState.lastSignal = { action: 'HOLD', score: 0, reason: 'off_session', session, close: candles[candles.length-1]?.close, time: Date.now() };
    }
  ...
  // Jika scanSignal ada dan lebih kuat, gabungkan untuk boost score
  if (scanSignal && scanSignal.action && scanSignal.action !== 'HOLD') {
    // Jika scanner setuju dengan signal level → boost score
    if (scanSignal.action === signal.action) {
      signal = { ...signal, score: Math.min(100, (signal.score || 50) + 10), boostedByScan: true };
    }
    // Jika signal HOLD tapi scanner kuat → override
    if (signal.action === 'HOLD' && scanSignal.score >= 65) {
