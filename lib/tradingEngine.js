...
export async function runCycle(candles, currentState = {}) {
  ...
  // Di autoPair mode: SCANNER adalah sumber utama arah (direction)
  // Level signal hanya memberi bobot konfirmasi
  if (scanSignal && scanSignal.action && scanSignal.action !== 'HOLD') {
    const levelAgreesWithScanner = signal.action === scanSignal.action;
    const levelScore  = signal.score  || 50;
    const scanScore   = scanSignal.score || 50;

    if (levelAgreesWithScanner) {
      // Keduanya setuju → combined score (70% scanner + 30% level)
      const combined = Math.round(scanScore * 0.7 + levelScore * 0.3);
      signal = {
        ...signal,
        action : scanSignal.action,
      ...
      // Konflik arah → scanner MENANG di autoPair (scanner adalah primary)
      // Level yang tidak setuju mengurangi score sedikit saja (-5)
      const penaltyScore = Math.max(50, scanScore - 5);
      signal = {
        ...signal,
        action : scanSignal.action,   // ← arah SELALU dari scanner
        score  : penaltyScore,
        overriddenByScan: true,
        levelConflict   : true,
        levelAction     : signal.action, // simpan arah level untuk info
      };
      addLog(`⚠️ Konflik: Scanner ${scanSignal.action} (${scanScore}) vs Level ${signal.action} (${levelScore}) → Scanner menang`, 'warning');
    }
  }
  ...
  // Double confirmation — reset jika instrument berubah (autoPair)
  if (botState._lastConfirmInstrument && botState._lastConfirmInstrument !== instrument) {
  ...
  const scannerHighConviction = (signal.overriddenByScan || signal.boostedByScan) && signal.score >= 72;
  ...
  if (allowed && (signal.action === 'BUY' || signal.action === 'SELL') && openPositions.length === 0 && signalConfirmed) {
    if (Date.now() - botState.lastActionTime < cooldownMs) {
      ...
      if (sizing.lots <= 0) {
        ...
        // ── Sanity check SL/TP setelah S/R adjustment ──
        // Pastikan SL dan TP tidak flip ke sisi yang salah
        if (isBuy) {
          if (finalSL >= close) finalSL = adaptive.stopLoss;   // reset ke adaptive
          if (finalTP <= close) finalTP = adaptive.takeProfit; // reset ke adaptive
        } else {
          if (finalSL <= close) finalSL = adaptive.stopLoss;
          if (finalTP >= close) finalTP = adaptive.takeProfit;
        }
        // Pastikan jarak minimal SL = 5 pips
        const minDist = pip * 5;
        if (isBuy  && (close - finalSL) < minDist) finalSL = close - pip * (s.stopLossPips || 30);
        if (!isBuy && (finalSL - close) < minDist) finalSL = close + pip * (s.stopLossPips || 30);
        if (isBuy  && (finalTP - close) < minDist) finalTP = close + pip * (s.takeProfitPips || 60);
        if (!isBuy && (close - finalTP) < minDist) finalTP = close - pip * (s.takeProfitPips || 60);
        ...
        // minRR lebih permisif: scanner override score tinggi bisa pakai 0.8
        const minRR = scannerHighConviction && signal.score >= 80 ? 0.8 :
                      activeMode === 'ultra_profit' ? 1.0 :
                      activeMode === 'ultra_light'  ? 2.0 : 1.1;
