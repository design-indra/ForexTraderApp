...
export async function runCycle(candles, currentState = {}) {
  ...
  if (scanSignal && scanSignal.action && scanSignal.action !== 'HOLD') {
    ...
    if (levelAgrees) {
      ...
      // Konflik: scanner menang tapi score dikurangi sedikit karena level tidak setuju
      const penaltyScore = Math.max(50, scanScore - 8);
      signal = {
        ...signal,
        action:           scanSignal.action,
        score:            penaltyScore,
        ...
        levelConflict:    true,
        levelAction:      signal.action,
      };
      addLog(`⚠️ Konflik: Scanner ${scanSignal.action} (${scanScore}) vs Level ${signal.action} → Scanner menang (${penaltyScore})`, 'warning');
    }
  }
