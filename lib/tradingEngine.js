...
export async function runCycle(candles, currentState = {}) {
  ...
  if (scanSignal && scanSignal.action && scanSignal.action !== 'HOLD') {
    ...
    if (levelAction === scanSignal.action) {
      ...
      if (scanScore >= 85) {
        // Scanner sangat dominan, level mungkin lagging — ikuti scanner
        signal = {
          ...signal,
          action:        scanSignal.action,
          score:         Math.round(scanScore * 0.85), // sedikit discount karena konflik
        ...
        // Konflik tanpa sinyal dominan → HOLD
        signal = {
          ...signal,
          action:      'HOLD',
          score:       50,
