...
export function checkSignalReversal(position, currentPrice, signal) {
  ...
  // FIX: Smart exit lebih konservatif — cegah BUY lalu SELL dalam waktu singkat
  // Syarat exit: (1) posisi sudah profit minimal 10 pips, (2) sinyal reversal sangat kuat,
  //              (3) posisi sudah berjalan minimal 15 menit
  const ageMinutes = position.openTime ? (Date.now() - position.openTime) / 60000 : 0;
  const minAge     = 15; // minimal 15 menit sebelum smart exit bisa trigger
  const minPips    = 10; // minimal profit 10 pips (naik dari 5)

  if (ageMinutes < minAge) return { shouldExit:false, pnlPips, pnlUSD }; // terlalu muda

  if (isBuy  && signal.action==='SELL' && signal.score < 25 && pnlPips > minPips)
    return { shouldExit:true, reason:'smart_signal_exit', pnlPips, pnlUSD };
  if (!isBuy && signal.action==='BUY'  && signal.score > 75 && pnlPips > minPips)
    return { shouldExit:true, reason:'smart_signal_exit', pnlPips, pnlUSD };
