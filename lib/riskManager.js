...
let runtimeSettings = {
  ...
  cooldownSeconds       : 60,       // MIFX: no scalping — cooldown lebih panjang
  minAccountUSD         : 10,
...
export function updateTrailingStop(position, currentPrice) {
  ...
  if (isBuy) {
    ...
    if (!position.trailingStop || newSL > position.trailingStop) {
      const updatedSL = Math.max(newSL, position.stopLoss);
      return { ...position, trailingStop:updatedSL, stopLoss:updatedSL };
    }
  } else {
    const newSL = currentPrice + trail;
    if (!position.trailingStop || newSL < position.trailingStop) {
      const updatedSL = Math.min(newSL, position.stopLoss);
      return { ...position, trailingStop:updatedSL, stopLoss:updatedSL };
    }
  }
  return position;
