...
export function calculateLotSize(balance, slPips, instrument='EUR_USD', botState={}, signalGrade='C') {
  ...
  // ── Max Profit Mode: ATR-dynamic → paksa pakai balance terkini + R:R min 1.5
  // Mode ini otomatis menggunakan adaptive SL/TP dari tradingEngine,
  // di sini kita naikkan riskPercent sedikit dan tandai agar minRR = 1.5
  if (mode === 'max_profit') riskPercent = Math.min(riskPercent * 1.2, 3.0);

  // ── Compound Mode: lot dihitung dari balance terkini (bukan start balance)
  // Efek: saat akun profit, lot otomatis naik proporsional
  // saat akun rugi, lot otomatis turun — melindungi modal
  const effectiveBalance = s.compoundEnabled
    ? balance                          // pakai balance terkini (sudah include profit/loss)
    : Math.min(balance, startBalance); // tanpa compound: cap di start balance agar lot stabil
