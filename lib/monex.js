 ...
 *  - Komisi $10/lot/sisi sudah diperhitungkan
 *  - Demo candle volatilitas realistis (bukan random besar)
 *  - Leverage 1:100
 */
...
// Komisi MIFX per lot per sisi
export const COMMISSION_PER_LOT = 10; // USD
...
/**
 * Hitung komisi round-trip (entry + exit) untuk MIFX Ultra Low
 * $10/lot × 2 sisi
 */
export function calculateCommission(lots) {
  return parseFloat((COMMISSION_PER_LOT * 2 * lots).toFixed(2));
}
...
// ── Demo candle generator — REALISTIS untuk MIFX ─────────────────────────────
const DEMO_PRICES = {
...
// Volatilitas realistis per 5 menit candle (dalam pips)
// Major pairs bergerak 3-8 pips per 5m candle di sesi normal
const REALISTIC_VOLATILITY_PIPS = {
  EUR_USD: 5,  GBP_USD: 7,  USD_JPY: 6,  AUD_USD: 5,
  USD_CAD: 5,  USD_CHF: 5,  EUR_GBP: 4,  EUR_JPY: 8,
  GBP_JPY: 10, AUD_JPY: 7,  EUR_AUD: 7,  GBP_AUD: 9,
  EUR_CHF: 4,  NZD_USD: 5,  XAU_USD: 150, XAG_USD: 30, // dalam harga bukan pips
};
...
function generateDemoCandles(instrument, count) {
  ...
  // Volatilitas realistis: 3-8 pips per candle untuk forex, lebih untuk Gold
  const volPips  = REALISTIC_VOLATILITY_PIPS[instrument] || 5;
  const isGold   = instrument === 'XAU_USD';
  const isJPY    = instrument.includes('JPY');

  // Volatility dalam harga
  const volatility = isGold ? 1.5 : isJPY ? pip * volPips * 1.5 : pip * volPips;

  // Buat trend yang realistis (bukan pure random)
  let trend = (Math.random() - 0.5) * 0.3; // trend kecil -0.15 to +0.15

  for (let i = count; i >= 0; i--) {
    // Trend berubah perlahan (mean reversion)
    trend = trend * 0.95 + (Math.random() - 0.5) * 0.1;
    trend = Math.max(-0.5, Math.min(0.5, trend));

    const trendMove = trend * volatility;
    const noise     = (Math.random() - 0.5) * volatility * 0.8;
    const move      = trendMove + noise;

    const open  = base;
    const close = parseFloat((open + move).toFixed(isJPY ? 3 : isGold ? 2 : 5));

    // High/Low realistis: 30-70% ekstra dari body
    const bodySize = Math.abs(close - open);
    const wickSize = bodySize * (0.3 + Math.random() * 0.7);
    const high = parseFloat((Math.max(open, close) + wickSize * Math.random()).toFixed(isJPY ? 3 : isGold ? 2 : 5));
    const low  = parseFloat((Math.min(open, close) - wickSize * Math.random()).toFixed(isJPY ? 3 : isGold ? 2 : 5));
