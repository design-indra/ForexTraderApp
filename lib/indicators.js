...
export function detectSupportResistance(candles, lookback = 20, threshold = 0.003) {
  // FIX BUG 2: threshold dinaikkan 0.001→0.003 (0.3%)
  // 0.001 = 0.1% dari EUR/USD = ~11 pip — terlalu sempit, nearSupport/Resistance hampir selalu false
  // 0.003 = 0.3% = ~33 pip — lebih realistis untuk deteksi proximity S/R
...
export function calculateMomentumScore(candles) {
  // FIX BUG 1: simetris BUY/SELL — penalty bearish sama besar dengan bonus bullish
  ...
  if (rsi !== null) {
    ...
    else if (rsi > 30 && rsi < 50) score -= 15; // bearish momentum (simetris)
    else if (rsi >= 70) score -= 5;              // overbought
    else if (rsi <= 30) score += 5;              // oversold — reversal potential
  }
  ...
  if (ema9 && ema21) {
    ...
    else score -= 10; // FIX: simetris, sebelumnya hanya -5
  }
  if (ema50) {
    if (closes[closes.length - 1] > ema50) score += 8;
    else score -= 8; // FIX: simetris, sebelumnya tidak ada penalty
  }
...
export function getHigherTFBias(candles) {
  // FIX BUG 3: EMA200 tidak valid karena bot hanya punya 100 candle
  // Ganti EMA200 dengan pendekatan struktur harga (higher highs/lows) yang valid dengan 100 candle
  ...
  // Struktur harga: bandingkan harga awal, tengah, dan akhir dari 100 candle
  const third   = Math.floor(closes.length / 3);
  const avgEarly = closes.slice(0, third).reduce((a, b) => a + b, 0) / third;
  const avgMid   = closes.slice(third, third * 2).reduce((a, b) => a + b, 0) / third;
  const avgLate  = closes.slice(third * 2).reduce((a, b) => a + b, 0) / (closes.length - third * 2);

  let score = 0;
  // EMA50 — valid dengan 100 candle
  if (ema50  && close > ema50)  score += 2;
  if (ema50  && close < ema50)  score -= 2;
  // EMA21 — momentum menengah
  if (ema21  && close > ema21)  score += 1;
  if (ema21  && close < ema21)  score -= 1;
  // Struktur: apakah harga secara umum naik atau turun selama 100 candle
  if (avgLate > avgMid && avgMid > avgEarly) score += 3; // uptrend
  if (avgLate < avgMid && avgMid < avgEarly) score -= 3; // downtrend
  // RSI bias
  if (rsi !== null) {
    if (rsi > 55) score += 1;
    if (rsi < 45) score -= 1;
...
export function candleConfirmation(candles, direction, n = 2) {
  ...
  // FIX BUG 5: tidak perlu SEMUA candle searah — cukup mayoritas 60%
  // Di 5m TF, candle sering bergantian meski trend jelas → 100% terlalu ketat
  const minCount = Math.ceil(n * 0.6); // 2 candle → min 2, 3 candle → min 2
...
export function normalizeScore(rawScore, source, action) {
  ...
  if (source === 'scanner') {
    // FIX MASALAH 7: scanner conviction 50-100, unified 0-100
    // BUY conviction 80 → unified 80 (langsung pakai)
    // SELL conviction 80 → unified 20 (flip: 100 - score)
    // Sebelumnya Math.min(50,...) membatasi SELL ke 0-50 — padahal harusnya bisa 0-50
    // Perbedaan: hapus Math.min(50,...) karena 100-rawScore sudah menghasilkan 0-50 untuk conviction 50-100
    if (action === 'BUY')  return Math.min(100, Math.max(50, rawScore));
    if (action === 'SELL') return Math.max(0, 100 - rawScore); // FIX: hapus Math.min(50) yang redundan
    return 50; // HOLD
  }
...
export function computeSignalScore({ rsi, ema9, ema21, macd, close, bb, trend }) {
  ...
  score = Math.max(0, Math.min(100, score));
  // FIX BUG 1: threshold diselaraskan dengan level3Signal (72/28) agar tidak entry di zona abu-abu
  const action = score >= 72 ? 'BUY' : score <= 28 ? 'SELL' : 'HOLD';
...
export function calculateAdaptiveTPSL(candles, close, direction = 'buy') {
  ...
  // FIX BUG 2: tpMult diturunkan 3.0→2.0 agar TP realistis di 5m TF
  // ATR 5m forex ~0.0008-0.0012 → tpMult=3.0 berarti 24-36 pips, jarang tercapai
  // tpMult=2.0 → 16-24 pips, lebih achievable, R:R tetap 1.33:1
  const slMult = 1.5;
  const tpMult = 2.0; // R:R = 1.33:1 — lebih sering TP tercapai
